...
from config3 import TERMINAL_PATH, ACCOUNT, PASSWORD, SERVER
...
# at module scope
bot = MT5AutoTrading(symbol='BTCUSD', magic=123456)

async def monitor_loop(symbol='BTCUSD', n_bars=100, pause_s=10):
    ...
    try:
        while True:
            ...
            if dir4h == -1:
                # in a 4h downtrend, look for bullish CHoCHs above fibo50 but below the 4h high
                s1 = df1h['choch_bull_price']
                s2 = df30m['choch_bull_price']
                valid1 = s1[(s1 > fibo4h_50) & (s1 < struct_high)]
                valid2 = s2[(s2 > fibo4h_50) & (s2 < struct_high)]
            else:
                # in a 4h uptrend, look for bearish CHoCHs below fibo50 but above the 4h low
                s1 = df1h['choch_bear_price']
                s2 = df30m['choch_bear_price']
                valid1 = s1[(s1 < fibo4h_50) & (s1 > struct_low)]
                valid2 = s2[(s2 < fibo4h_50) & (s2 > struct_low)]

            # 2) Build a combined hits DataFrame with timeframe and actual timestamp
            df1_hits = valid1.rename("price").to_frame().assign(
                tf="1h",
                time=df1h.loc[valid1.index, "time"]
            )
            df30_hits = valid2.rename("price").to_frame().assign(
                tf="30m",
                time=df30m.loc[valid2.index, "time"]
            )
            combined = pd.concat([df1_hits, df30_hits])

            if combined.empty:
                # no valid CHoCH — bail out
                await asyncio.sleep(pause_s)
                continue
                
            # 3) Pick the “best” CHoCH depending on trend direction
            if dir4h == -1:
                best_idx = combined['price'].idxmax()
                print(f"[DEBUG] Downtrend: idxmax → {best_idx}")
            else:
                best_idx = combined['price'].idxmin()
                print(f"[DEBUG] Uptrend: idxmin → {best_idx}")
            

            rows    = combined.loc[best_idx]
            print(f"[DEBUG] rows at best_idx:\n{rows}")

            # 3) If you got a small DataFrame, pick the correct row by comparing prices
            if isinstance(rows, pd.DataFrame):
                arr = rows['price'].to_numpy()
                if dir4h == -1:
                    # downtrend → pick the maximum price among these duplicates
                    pick_pos = arr.argmax()
                else:
                    # uptrend   → pick the minimum price among these duplicates
                    pick_pos = arr.argmin()
                best_row = rows.iloc[pick_pos]
                print(f"[DEBUG] picked row at position {pick_pos}:\n{best_row}")
            else:
                # it was already a single Series
                best_row = rows
                print(f"[DEBUG] single-row pick:\n{best_row}")


            # 4) Extract your values
            choch_idx   = best_idx
            choch_price = float(best_row['price'])
            tf_ch       = best_row['tf']
